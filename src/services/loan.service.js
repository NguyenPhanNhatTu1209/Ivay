	...
	try {
		...
		if (loan) {
			if (devices) {
				if (status === DFStatusLoan.accept)
					pushMultipleNotification(
						'Yêu cầu vay của bạn đã được phê duyệt',
						'Khoản vay của bạn sẽ được giải ngân trong thời gian sớm nhất',
						"",
						{
							action: 'ACCEPT'
						},
					...
					pushMultipleNotification(
						'Yêu cầu vay của bạn đã bị từ chối',
						'Hãy liên hệ CSKH để biết chi tiết lý do',
						"",
						{
							action: 'REJECT'
						},
